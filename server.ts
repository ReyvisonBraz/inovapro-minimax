...
async function startServer() {
  ...
  app.post("/api/client-payments", (req, res) => {
    ...
    const result = db.prepare(`
      ...
      (customerId, description, totalAmount, paidAmount, purchaseDate, dueDate, paymentMethod, status, installmentsCount, type)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      customerId, description, totalAmount, paidAmount || 0, 
      purchaseDate, dueDate, paymentMethod, status || 'pending', 
      installmentsCount || 1, type || 'income'
    );
    res.json({ id: result.lastInsertRowid });
  });

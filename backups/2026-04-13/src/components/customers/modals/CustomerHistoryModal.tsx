        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            <div className="flex items-start justify-between mb-4 md:mb-6">
              <div>
                <h3 className="text-lg md:text-xl font-bold">Histórico do Cliente</h3>
              ...
              <button 
                onClick={onClose}
                className="p-2 rounded-xl hover:bg-white/5 text-slate-500 hover:text-white transition-all shrink-0"
              >
            ...
            <div className="flex-1 overflow-y-auto pr-2 space-y-4 custom-scrollbar">
              {historyItems.length > 0 ? (
                historyItems.map((item) => (
                  <div 
                    key={item.id}
                    className="bg-white/5 border border-white/10 rounded-2xl p-3 md:p-4 flex flex-col sm:flex-row items-start gap-3 md:gap-4 hover:bg-white/[0.07] transition-all"
                  >
                    <div className={cn(
                      "p-2 md:p-3 rounded-xl shrink-0",
                      item.type === 'payment' ? "bg-emerald-500/10 text-emerald-500" : "bg-primary/10 text-primary"
                    )}>
                      {item.type === 'payment' ? <ReceiptText size={16} className="md:w-5 md:h-5" /> : <Wrench size={16} className="md:w-5 md:h-5" />}
                    </div>
                    ...
                    <div className="flex-1 min-w-0 w-full">
                      ...
                      <p className="text-xs md:text-sm text-slate-400 line-clamp-2 mb-3">{item.details}</p>
            ...
            <div className="mt-6 md:mt-8 pt-4 md:pt-6 border-t border-white/10 flex justify-end">
              <button 
                onClick={onClose}
                className="w-full sm:w-auto px-8 py-3 rounded-xl font-bold text-slate-500 hover:bg-white/5 transition-all text-sm uppercase tracking-widest"
              >
                Fechar
              </button>
            </div>
